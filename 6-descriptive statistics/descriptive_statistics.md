...
![Examples of data distributions](image_aa8bd0.png)
*(위 이미지는 가우시안, t, F, 카이제곱 분포의 예시입니다.)*
...
![modal.png](modal.png)
*(위 이미지는 단일 모드 및 이중/다중 모드 분포의 예시입니다.)*
...
* **가우시안 분포 (Gaussian distribution)**: 가장 널리 사용되는 대칭적인 종 모양 분포입니다.
* **t-분포 (t-distribution)**: 표본 크기가 작을 때 사용되는, 정규 분포와 유사하지만 꼬리가 두꺼운 분포입니다.
* **F-분포 (F-distribution)**: 두 분산의 비율을 비교할 때 사용되며, 오른쪽으로 치우친 분포입니다.
* **카이제곱 분포 (Chi-square, χ²)**: 범주형 데이터 분석에 사용되며, 오른쪽으로 치우친 분포입니다.
...
하이퍼파라미터는 학습의 다양한 측면을 제어하는 변수

- 학습률
- 배치 크기
- 에포크

**매개변수는 가중치, 편향과 같이 모델 자체의 변수(모델이 계산)이고 하이퍼파라미터는 사용자가 제어하는 변수**

### 학습률

모델이 수렴하는 속도 → 경사하강 프로세스에서 가중치와 편향을 변경하는 정도를 결정한다.

### 배치 크기

- 모델이 가중치와 편향을 업데이트 하기 전에 처리하는 예시의 갯수

데이터 세트가 수십 수백만개 이상일경우, 일괄적으로 처리하기 힘들기 때문에 배치 크기를 결정한다.

1. 확률적 경사하강법(SGD)
    - 반복당 하나의 예시만 사용한다.
    - 무작위로 데이터세트를 선택한다.
    - 노이즈(손실이 감소하지 않고 증가)가 많이 발생한다.
2. 미니배치 SGD
    - 전체배치와 SGD의 절충안
    - 예시의 갯수를 1 ~ N 사이로 결정
    - 결정된 갯수를 무작위로 선택하여 손실을 줄인다.

노이즈가 무조건적으로 제거해야 하는 특성은 아니다 → 후반 모듈에서 설명

### 에포크

에포크는 모델이 학습 세트의 모든 예시를 한번 처리했음을 의미

- 1000개의 예시 갯수에서 배치가 100인 학습 세트가 주어지면 하나의 에포크를 완료하는데 10번의 에포크가 필요
- 일반적으로 에포크 수가 높으면 모델이 더 좋지만 시간이 더 걸림